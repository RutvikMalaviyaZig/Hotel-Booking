...
const ListRoom = () => {
    ...
    const toggleRoomAvailability = async (roomId) => {
        try {
            const { data } = await axios.post(`/api/room/availability`, {
                roomId,
            }, {
                headers: {
                    Authorization: `Bearer ${await getToken()}`,
    ...
    return (
        <div>
            <Title title="Room Listing" subtitle="Manage your rooms" align="left" font="outfit" />
                    ...
                    <tbody className='text-sm'>
                        {rooms.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className='py-3 px-4 max-sm:hidden border-t border-gray-300'>{item.amenities.join(', ')}</td>
                                ...
                                <td className='py-3 px-4 border-t border-gray-300 text-sm text-red-500 text-center'>
                                    <label className="relative inline-flex items-center cursor-pointer text-gray-900 gap-3">
                                        <input type="checkbox" className="sr-only peer" checked={item.isAvailable} onChange={() => toggleRoomAvailability(item._id)} />
